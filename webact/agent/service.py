...
if TYPE_CHECKING:
	from webact.skills.views import Skill

from dotenv import load_dotenv

from webact.agent.cloud_events import (
...
from webact.agent.message_manager.utils import save_conversation
from webact.llm.base import BaseChatModel
from webact.llm.exceptions import ModelProviderError, ModelRateLimitError
from webact.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from webact.tokens.service import TokenCost
...
from webact import Browser, BrowserProfile, BrowserSession
from webact.agent.judge import construct_judge_messages

# Lazy import for gif to avoid heavy agent.views import at startup
# from webact.agent.gif import create_history_gif
from webact.agent.message_manager.service import (
	MessageManager,
)
from webact.agent.prompts import SystemPrompt
from webact.agent.views import (
	ActionResult,
	AgentError,
	AgentHistory,
	AgentHistoryList,
...
from webact.browser.events import _get_timeout
from webact.browser.session import DEFAULT_BROWSER_PROFILE
from webact.browser.views import BrowserStateSummary
from webact.config import CONFIG
from webact.dom.views import DOMInteractedElement, MatchLevel
from webact.filesystem.file_system import FileSystem
from webact.observability import observe, observe_debug
from webact.telemetry.service import ProductTelemetry
from webact.telemetry.views import AgentTelemetryEvent
from webact.tools.registry.views import ActionModel
from webact.tools.service import Tools
from webact.utils import (
	URL_PATTERN,
	_log_pretty_path,
...
class Agent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from webact.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from webact import ChatBrowserUse
		...
		if skill_service is not None:
			...
			from webact.skills import SkillService
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'webact.Agent🅰 {_task_id} ⇢ 🅑 {_browser_session_id} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from webact.screenshots.service import ScreenshotService
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from webact.llm.messages import AssistantMessage, UserMessage
	...
	async def run(
		...
		from webact.utils import SignalHandler
	...
	async def _generate_rerun_summary(
		...
		from webact.agent.views import RerunSummaryAction
		...
		from webact.agent.prompts import get_rerun_summary_message, get_rerun_summary_prompt
		...
		try:
			...
			from webact.llm.messages import BaseMessage
	...
	async def _execute_ai_step(
		...
		"""
		from webact.agent.prompts import get_ai_step_system_prompt, get_ai_step_user_prompt, get_rerun_summary_message
		from webact.llm.messages import SystemMessage, UserMessage
		from webact.utils import sanitize_surrogates
		...
		try:
			from webact.dom.markdown_extractor import extract_clean_markdown
	...
	def detect_variables(self) -> dict[str, DetectedVariable]:
		...
		from webact.agent.variable_detector import detect_variables_in_history
