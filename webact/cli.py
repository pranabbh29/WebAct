...
if 'init' in sys.argv:
	from webact.init_cmd import INIT_TEMPLATES
	from webact.init_cmd import main as init_main
...
from webact import Agent, Controller
from webact.agent.views import AgentSettings
from webact.browser import BrowserProfile, BrowserSession
from webact.logging_config import addLoggingLevel
from webact.telemetry import CLITelemetryEvent, ProductTelemetry
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('webact.'):
		...
		browser_use_logger = logging.getLogger('webact')
		...
		for logger_name in ['webact.Agent', 'webact.controller', 'webact.agent', 'webact.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'webact.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('webact.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('webact.app')
...
async def textual_interface(config: dict[str, Any]):
	...
	logger = logging.getLogger('webact.startup')
	...
	try:
		...
		try:
			from webact.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	from webact.sync.auth import DeviceAuthClient
	...
	try:
		...
		from webact.sync.service import CloudSync
		...
		try:
			...
			from webact.utils import create_task_with_error_handling
		...
		if task_id and sync_service:
			try:
				from webact.agent.cloud_events import UpdateAgentTaskEvent
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from webact.mcp.server import main as mcp_main
