...
from webact.browser.cloud.cloud import CloudBrowserAuthError, CloudBrowserClient, CloudBrowserError
...
from webact.browser.cloud.views import CloudBrowserParams, CreateBrowserRequest, ProxyCountryCode
from webact.browser.events import (
...
from webact.browser.profile import BrowserProfile, ProxySettings
from webact.browser.views import BrowserStateSummary, TabInfo
from webact.dom.views import DOMRect, EnhancedDOMTreeNode, TargetInfo
from webact.observability import observe_debug
from webact.utils import _log_pretty_url, create_task_with_error_handling, is_new_tab_page

if TYPE_CHECKING:
	from webact.actor.page import Page
	from webact.browser.demo_mode import DemoMode
	from webact.browser.watchdogs.captcha_watchdog import CaptchaWaitResult
...
class BrowserSession(BaseModel):
	...
	def from_system_chrome(cls, profile_directory: str | None = None, **kwargs: Any) -> Self:
		...
		from webact.skill_cli.utils import find_chrome_executable, get_chrome_profile_path, list_chrome_profiles
	...
	def list_chrome_profiles(cls) -> list[dict[str, str]]:
		...
		from webact.skill_cli.utils import list_chrome_profiles
	...
	def logger(self) -> Any:
		...
		# 	self._logger = logging.getLogger(f'webact.{self}')
		return logging.getLogger(f'webact.{self}')
	...
	def model_post_init(self, __context) -> None:
		...
		from webact.browser.watchdog_base import BaseWatchdog
	...
	async def kill(self) -> None:
		...
		from webact.browser.events import SaveStorageStateEvent
	...
	async def new_page(self, url: str | None = None) -> 'Page':
		...
		from webact.actor.page import Page as Target
	...
	async def get_pages(self) -> list['Page']:
		...
		# Import here to avoid circular import
		from webact.actor.page import Page as PageActor
	...
	async def attach_all_watchdogs(self) -> None:
		...
		from webact.browser.watchdogs.aboutblank_watchdog import AboutBlankWatchdog
		from webact.browser.watchdogs.captcha_watchdog import CaptchaWatchdog

		# from webact.browser.crash_watchdog import CrashWatchdog
		from webact.browser.watchdogs.default_action_watchdog import DefaultActionWatchdog
		from webact.browser.watchdogs.dom_watchdog import DOMWatchdog
		from webact.browser.watchdogs.downloads_watchdog import DownloadsWatchdog
		from webact.browser.watchdogs.har_recording_watchdog import HarRecordingWatchdog
		from webact.browser.watchdogs.local_browser_watchdog import LocalBrowserWatchdog
		from webact.browser.watchdogs.permissions_watchdog import PermissionsWatchdog
		from webact.browser.watchdogs.popups_watchdog import PopupsWatchdog
		from webact.browser.watchdogs.recording_watchdog import RecordingWatchdog
		from webact.browser.watchdogs.screenshot_watchdog import ScreenshotWatchdog
		from webact.browser.watchdogs.security_watchdog import SecurityWatchdog
		from webact.browser.watchdogs.storage_state_watchdog import StorageStateWatchdog
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		if not self.cdp_url.startswith('ws'):
			...
			async with httpx.AsyncClient(timeout=httpx.Timeout(30.0), trust_env=not is_localhost) as client:
				...
				from webact.utils import get_browser_use_version
		...
		try:
			...
			from webact.browser.session_manager import SessionManager
			...
			from webact.utils import is_new_tab_page
	...
	async def navigate_to(self, url: str, new_tab: bool = False) -> None:
		...
		"""
		from webact.browser.events import NavigateToUrlEvent
	...
	async def get_dom_element_at_coordinates(self, x: int, y: int) -> EnhancedDOMTreeNode | None:
		...
		"""
		from webact.dom.views import NodeType
	...
	def find_file_input_near_element(
		...
		"""
		from webact.dom.views import EnhancedDOMTreeNode
