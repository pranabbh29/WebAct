...
from webact.browser.profile import BrowserProfile
from webact.browser.session import BrowserSession
...
async def test_cdp_client_headers_passed_on_connect():
	...
	with patch('webact.browser.session.CDPClient') as mock_cdp_client_class:
		...
		# Mock SessionManager (imported inside connect() from webact.browser.session_manager)
		with patch('webact.browser.session_manager.SessionManager') as mock_session_manager_class:
...
async def test_headers_used_for_json_version_endpoint():
	...
	with patch('webact.browser.session.httpx.AsyncClient') as mock_client_class:
		...
		with patch('webact.browser.session.CDPClient') as mock_cdp_client_class:
			...
			with patch('webact.browser.session_manager.SessionManager') as mock_sm_class:
				mock_sm = MagicMock()
