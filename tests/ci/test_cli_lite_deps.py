...
def test_cli_entry_point(lite_venv):
	...
	result = subprocess.run(
		[str(lite_venv), '-m', 'webact.skill_cli.main', '--help'],
		capture_output=True,
		text=True,
		timeout=30,
	)
...
def test_daemon_imports(lite_venv):
	...
	result = _import_check(lite_venv, 'from webact.skill_cli.daemon import main')
...
def test_browser_session_imports(lite_venv):
	...
	result = _import_check(lite_venv, 'from webact.browser.session import BrowserSession')
