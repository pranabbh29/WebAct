...
### Post-Install
```bash
webact doctor   # Validate installation
webact setup    # Run setup wizard (optional)
```

### Generate Templates
```bash
webact init                          # Interactive template selection
webact init --list                   # List available templates
webact init --template basic         # Generate specific template
webact init --output my_script.py    # Specify output file
webact init --force                  # Overwrite existing files
```
...
webact input 3 "john@example.com"

# Take a screenshot
webact screenshot output.png
...
webact --headed open https://example.com

# Use your real Chrome with Default profile (with existing logins/cookies)
webact --profile "Default" open https://gmail.com

# Use a specific Chrome profile
webact --profile "Profile 1" open https://gmail.com

# Auto-discover and connect to running Chrome
webact --connect open https://example.com

# Connect to an existing browser via CDP URL
webact --cdp-url http://localhost:9222 open https://example.com

# WebSocket CDP URL also works
webact --cdp-url ws://localhost:9222/devtools/browser/... state
```
...
```bash
webact python "x = 42"           # Set variable
webact python "print(x)"         # Access variable (prints: 42)
webact python "print(browser.url)"  # Access browser
webact python --vars             # Show defined variables
webact python --reset            # Clear namespace
webact python --file script.py   # Run Python file
```
...
webact cloud login sk-abc123...

# Provision a cloud browser and connect
webact cloud connect
webact state                    # works normally
webact close                    # disconnects AND stops cloud browser

# List browsers
webact cloud v2 GET /browsers

# Create a task
webact cloud v2 POST /tasks '{"task":"Search for AI news","url":"https://google.com"}'

# Poll until done
webact cloud v2 poll <task-id>
...
webact cloud connect                  # Provision cloud browser
webact open https://abc.trycloudflare.com
```
...
webact open https://example.com           # uses session 'default'
webact state                              # talks to 'default' daemon

# Named sessions
webact --session work open https://example.com
webact --session work state
...
webact --session work close

# Close all sessions
webact close --all

# Env var fallback
BROWSER_USE_SESSION=work webact state
```
...
### Fill a Form
```bash
webact open https://example.com/contact
webact state
# Shows: [0] input "Name", [1] input "Email", [2] button "Submit"
webact input 0 "John Doe"
webact input 1 "john@example.com"
webact click 2
```

### Extract Data with JavaScript
```bash
webact open https://news.ycombinator.com
webact eval "Array.from(document.querySelectorAll('.titleline a')).slice(0,5).map(a => a.textContent)"
```
...
5. Daemon auto-starts when needed, auto-exits when browser dies, or stops with `webact close`
...
# Or run via Git Bash
& "C:\Program Files\Git\bin\bash.exe" -c 'webact --help'
```

### "Failed to start daemon" error
Kill zombie processes:
```powershell
# Find webact Python processes
